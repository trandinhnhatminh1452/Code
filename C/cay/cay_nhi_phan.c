...
void load(char* filename){
    ...
    while(1){
        ...
        if(root == NULL) root = makeNode(u); //creat the root
        int l,r;
        fscanf(f,"%d%d",&l,&r);
...
void printTree(Node* r){
    if(r==NULL) return;
    printf("%d: ",r->id);
    if(r->leftChild==NULL) printf("leftChilde = NULL");
    else printf("leftChild = %d",r->rightChild->id);
    printf("\n");
...
void processHeight(){
    ...
    Node*p=Find(root,id);
    if(p == NULL) printf("Not found %d\n",id);
    else printf("Height of %d is %d",id,height(p));
}
...
void main(){
    while(1){
        ...
        else if(strcmp(cmd,"print")==0) processLoad();
